...
자동화 테스트는 전체 통합 스위트 형태는 아니지만, `test_scripts/` 아래에 집중형 `unittest` 스크립트 14개가 포함되어 있다. 이미지 업로드 API, 메시지 첨부 저장, 멀티모달 payload 구성, 첨부 파일 TTL cleanup 외에 웹 검색, usage(Free/Pro 계정), auth 로깅, 모델 매핑, 사용자 scope 히스토리, 스트림 오류 처리, LiteLLM 호환성, API 통합 등을 검증한다.

`.env` 파일은 없어도 서버는 실행되지만 정상 운영 시 아래 두 값은 넣는 것을 권장한다. `COPILOT_ENVELOPE_SECRET` 이 없으면 서버를 재시작할 때마다 모든 envelope 가 무효화된다. `TAVILY_API_KEY` 가 없으면 웹 검색 기능이 동작하지 않는다 (서버 기동 시 WARNING 로그만 남기고 빈 결과를 반환하며, 채팅 자체는 계속된다). GitHub/Copilot 토큰은 절대 `.env` 에 직접 넣지 말 것 — 인증은 브라우저 내 GitHub device flow 로 이루어진다. `main` 브랜치에 push 하면 `.github/workflows/deploy.yml` 이 Oracle Cloud 로 자동 배포한다 (`/home/ubuntu/copilot_chat` 진입 → `git pull` → `sudo systemctl restart copilot-chat`).
...
`main.py` 는 의도적으로 얇게 유지된다. 라우트 핸들러, 서비스 예외(`CopilotAuthError`, `CopilotChatRequestError`, `ConversationStateError`, `ImageAttachmentError`)와 `RequestValidationError` 를 정제된 `{code, message}` JSON 으로 변환하는 5개의 예외 핸들러, attachment payload 에 서명된 `contentUrl` 을 주입하는 `_decorate_attachment_urls`, 그리고 binding 쿠키를 발급/회전시키고 거기서 conversation scope id 를 파생시키는 `_resolve_browser_session_context` 헬퍼로 구성된다. **인증이 필요한 모든 엔드포인트는 이 헬퍼를 거쳐야 하며, 우회하지 말 것.**

### 서비스 레이어 (`services/`)

`main.py` 에서 싱글턴으로 배선되는 4개의 서비스:
...
- **`ImageAttachmentService`** (`image_attachment_service.py`) — 업로드 이미지의 바이트 검증과 저장, 서명된 첨부 URL 생성/검증, 파일 삭제를 담당한다. 현재 서버는 `image/jpeg` 만 허용하고 1개당 5MB 이하만 저장한다. 첨부 이미지는 `.uploads/images/` 아래에 저장되며, 브라우저에 노출되는 것은 실제 파일 경로가 아니라 현재 브라우저 세션 secret 으로 서명된 `contentUrl` 뿐이다. 이 URL 의 토큰 TTL 은 10분이다.
...
2. **Pro / paid** (`quota_snapshots.chat`, `quota_snapshots.premium_interactions`) — 응답에 `quota_snapshots` 객체가 있고 그 안에 `chat`, `premium_interactions`, `completions` 중 하나 이상의 키가 있을 때 이 경로로 진입한다. 각 metric 의 `remaining` 을 잔여량, `entitlement` 를 총량(`total`)으로 매핑하고 `used = total - remaining` 으로 계산한다. `unlimited == true` 인 metric 은 remaining/total/used 를 모두 null 로 처리하고 `무제한` 으로 표시한다.
...
- `COPILOT_CHAT_HISTORY_DB_PATH` → `.copilot_chat_history.sqlite3` (가시 트랜스크립트 + attachment metadata)

첨부 이미지 바이너리 파일 자체는 DB 밖의 `.uploads/images/` 에 저장된다.
